...
bool FeatureIndex::buildUnigramFeature(LearnerPath *path,
  ...
  feature_.clear();
  strncpy_s(ubuf.get(), sizeof(ubuf.get()), ufeature, ubuf.size());
...
bool FeatureIndex::buildBigramFeature(LearnerPath *path,
  ...
  feature_.clear();
  strncpy_s(lbuf.get(), sizeof(lbuf.get()), rfeature, lbuf.size());
  strncpy_s(rbuf.get(), sizeof(rbuf.get()), lfeature, rbuf.size());
...
bool FeatureIndex::convert(const Param &param,
  ...
  strncpy_s(charset_buf, sizeof(charset_buf), to.c_str(), 31);
