...
bool partial_match(const char *f1, const char *f2) {
  ...
  strncpy_s(buf1.get(), sizeof(buf1.get()), f1, buf1.size());
  strncpy_s(buf2.get(), sizeof(buf2.get()), f2, buf2.size());
...
bool Viterbi::initPartial(Lattice *lattice) {
  ...
  strncpy_s(str,sizeof(str), lattice->sentence(), lattice->size() + 1);
